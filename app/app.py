...
# ---- Fixed likely_homoglyph: safe regex and heuristics ----
def likely_homoglyph(domain: str) -> bool:
    """
    Heuristic tests for suspicious domains:
      - mixed letters+digits in odd places
      - lots of hyphens/subdomains
      - many domain parts (e.g., long subdomain chains)
      - repeated character runs (e.g., '!!' or '---') detected safely
    ...
    # contains both letters and digits (often used to obfuscate)
    if any(ch.isdigit() for ch in domain) and any(ch.isalpha() for ch in domain):
        return True

    # many hyphens is suspicious (e.g., 'secure-payments-bank--verify.example.com')
    if domain.count("-") >= 2:
        return True

    # unusually many domain segments (subdomain abuse)
    if len(domain.split(".")) > 3:
        return True

    # detect repeated non-word/non-dot characters safely, e.g., '!!!' or '---'
    if re.search(r"([^\w\.])\1{1,}", domain):
        return True
...
def analyze_triggers_scored(text: str):
    ...
    # sender/reply-to mismatch hints (best-effort)
